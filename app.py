...
MASTER_PROMPT = (
    ...
    "RULES:\n"
    "1. THE LEDGER: Read context provided. Address THIS specific user's patterns. \n"
    "2. NO CHEAP PRAISE: Acknowledge experience as a forge.\n"
...
with st.sidebar:
    ...
    if st.button("Bow-Out (Save & Clear)", use_container_width=True):
        if len(st.session_state.msgs) > 2:
            ...
            payload = {"model": "llama-3.1-8b-instant", "messages": [{"role": "system", "content": "Summarize user growth. One dense paragraph."}, {"role": "user", "content": chat_log}], "temperature": 0.3}
